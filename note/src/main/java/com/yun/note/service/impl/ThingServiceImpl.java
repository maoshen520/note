...
//    private INoteThingLogDao noteThingLogDao;  //笔记小记日志的数据库接口
    ...
    public List<Thing> getUserNormalThing(String search, Integer filter, int userId) throws ServiceException {
        ...
        QueryWrapper wrapper = QueryWrapper.create()
                .select(Tables.THING.ID, Tables.THING.TITLE, Tables.THING.TOP, Tables.THING.TAGS, Tables.THING.UPDATE_TIME, Tables.THING.FINISHED)
                .where(Tables.THING.USER_ID.eq(userId))
                .and(Tables.THING.STATUS.eq(1));

        //  是否有关键字查询
        if (Validator.isNotEmpty((search))){
            wrapper.and(
                    Tables.THING.TITLE.like(search)
                    .or(Tables.THING.TAGS.like(search))
            );
        }

        //是否有过来条件(finished字段)
        if (filter != null && (filter == 0 || filter == 1)) {
            wrapper.and(Tables.THING.FINISHED.eq(filter));
        }

        // 排序
        wrapper.orderBy(Tables.THING.FINISHED.asc(), Tables.THING.TOP.desc(), Tables.THING.UPDATE_TIME.desc());
    ...
    public void topThing(boolean isTop, int thingId, int userId) throws ServiceException {
        ...
        QueryWrapper wrapper = QueryWrapper.create()
                .where(Tables.THING.ID.eq(thingId))
                .and(Tables.THING.USER_ID.eq(userId))
                .and(Tables.THING.STATUS.eq(1))
                .and(Tables.THING.TOP.eq(beforeTop));
    ...
    public Thing getEditThing(int thingId, int userId) throws ServiceException {
        ...
        QueryWrapper wrapper = QueryWrapper.create()
                .select(Tables.THING.TITLE, Tables.THING.TOP, Tables.THING.TAGS, Tables.THING.CONTENT, Tables.THING.USER_ID)
                .where(Tables.THING.ID.eq(thingId))
                .and(Tables.THING.USER_ID.eq(userId))
                .and(Tables.THING.STATUS.eq(1));
    ...
    public void updateThing(Thing thing) throws ServiceException {
        ...
        //修改小记的条件
        QueryWrapper wrapper = QueryWrapper.create()
                .where(Tables.THING.ID.eq(thing.getId()))
                .and(Tables.THING.USER_ID.eq(thing.getUserId()))
                .and(Tables.THING.STATUS.eq(1));
